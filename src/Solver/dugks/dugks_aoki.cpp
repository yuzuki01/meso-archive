...
/// 构造函数
SCell::Cell(int cell_id, DUGKS_AOKI *Solver) {
    id = cell_id;
    solver = Solver;
...
SFace::Interface(int face_id, DUGKS_AOKI *Solver) {
    id = face_id;
    solver = Solver;
    int dvs_num = solver->DVS.NELEM;
    density = temperature = 0.0;
...
void Scheme::init() {
    ...
    for (auto &cell : mesh.CELLS) {
        ...
        for (int i = 0; i < DVS.NELEM; i++) {
            ...
            double g = g_eq(rho, T, particle.pos, Vector(0.0, 0.0, 0.0));
            double h = h_eq(g, T);
            scheme_cell.g_t[i] = g;
            scheme_cell.h_t[i] = h;
            if (cell.have_shadow_cell) {
...
/// 平衡态函数
double
Scheme::g_eq(double density, double temperature, const Vector &particle_velocity, const Vector &flow_velocity) const {
    Vector c = particle_velocity - flow_velocity;
    double RT2 = 2.0 * R * temperature;
    return density * exp(-(c * c) / RT2) / (RT2 * Pi);     /// only for 2D case
}

double
Scheme::h_eq(double geq, double temperature) const {
    return geq * temperature * R;
}

/// 松弛时间函数
double Scheme::tau_f(double density) const {
    return 1.0 / (Ac * density);
}

/// 控制体算法函数
void SCell::update_f_bp() {
    double tau = solver->tau_f(density);
    double C = 3.0 * solver->half_dt / (2.0 * tau + solver->dt);
    for (int i = 0; i < solver->DVS.NELEM; i++) {
        auto &particle = solver->DVS.CELLS[i];
        double geq = solver->g_eq(density, temperature, particle.pos, velocity);
        g_bp[i] = (1.0 - C) * g_t[i] + C * geq;
        h_bp[i] = (1.0 - C) * h_t[i] + C * solver->h_eq(geq, temperature);
    }
    // shadow mesh
...
void SCell::update_macro_var() {
    ...
    for (int i = 0; i < solver->DVS.NELEM; i++) {
        ...
        temperature += particle.volume * ((c * c) * g_t[i] + h_t[i]);
    }
    temperature /= 3.0 * density * solver->R;
...
void SFace::update_macro_var() {
    ...
    for (int i = 0; i < solver->DVS.NELEM; i++) {
        auto &particle = solver->DVS.CELLS[i];
        Vector c = particle.pos - velocity;
        temperature += particle.volume * ((c * c) * g_b[i] + h_b[i]);
    }
    temperature /= 3.0 * density * solver->R;
}
...
void SFace::update_f() {
    ...
    for (int i = 0; i < solver->DVS.NELEM; i++) {
        auto &particle = solver->DVS.CELLS[i];
        double geq = solver->g_eq(density, temperature, particle.pos, velocity);
        g[i] = (1.0 - C) * g_b[i] + C * geq;
        h[i] = (1.0 - C) * h_b[i] + C * solver->h_eq(geq, temperature);
    }
}
            ...
            /// "isothermal_wall"   3
        {
            ...
            for (int i = 0; i < solver->DVS.NELEM; i++) {
                ...
                if (kn < 0.0) {
                    ...
                    rho_w_reflect +=
                            particle.volume * kn * solver->g_eq(1.0, mark.temperature, particle.pos, mark.velocity);
                }
            }
            rho_w /= rho_w_reflect;
            for (int i = 0; i < solver->DVS.NELEM; i++) {
                auto &particle = solver->DVS.CELLS[i];
                if (particle.pos * nv >= 0.0) {
                    g[i] = solver->g_eq(rho_w, mark.temperature, particle.pos, mark.velocity);
...
void Scheme::do_save() {
    ...
    switch (mesh.dimension()) {
        case 2:
            fp << GEOM::tecplot_file_header(mesh, {"density", "temperature", "velocity-x", "velocity-y"});
            break;
        case 3:
            fp << GEOM::tecplot_file_header(mesh, {"density", "temperature", "velocity-x", "velocity-y", "velocity-z"});
            break;
        default:
            fp << "Error dimension.";
            fp.close();
...
void Scheme::do_crashed(DUGKS_AOKI::Cell &cell) {
    is_crashed = true;
